  ...
  <div class="main-box">
    ...
    <div class="table-box">
      <ProTable
        ref="proTable"
        :columns="columns"
        :request-api="getSensitive_dirList"
...
import { Sensitive_dir } from "@/api/interface";
...
import { getSensitive_dirList, deleteSensitive_dir } from "@/api/modules/sensitive_dir";
...
// 表格配置项
const columns = reactive<ColumnProps<Sensitive_dir.ResSensitive_dirList>[]>([
...
// 删除资产
const deleteAccount = async (params: Sensitive_dir.ResSensitive_dirList) => {
  await useHandleData(deleteSensitive_dir, { id: [params.id] }, `删除【${params.info}】资产`);
  proTable.value?.getTableList();
};
// 批量删除资产
const batchDelete = async (id: string[]) => {
  await useHandleData(deleteSensitive_dir, { id }, "删除所选用户信息");
